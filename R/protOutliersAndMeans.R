...
#'================================================================
OutlierIdentify <- function(protClass, refCol, nCol, save.data = TRUE, byPeptide = FALSE) {
...
geneProfileSummary <- function(protsCombineCnew, outlier, refCol, nCol, eps, dataUse, save.data = TRUE) {
  ...
  for(i in 1:(length(name))) {
    ...
    output <- output %>%
      mutate(!!paste0(quo_name(channel), ".MU") := ifelse(!!as.name(paste0(quo_name(channel), ".singular")) == "FALSE",
                                                          !!as.name(paste0(quo_name(channel), ".coef")),
                                                          !!as.name(paste0(quo_name(channel), ".all.mu"))),
             !!paste0(quo_name(channel), ".SE") := ifelse(!!as.name(paste0(quo_name(channel), ".singular")) == "FALSE",
