...
AcupMarkers <- relAmtTransform(markerProfiles, NstartMaterialFractions=6, totProt=totProtAT5)
```
...
markerProfilesrsa <- RSAfromS(SS=markerProfiles, NstartMaterialFractions=6,
        totProt=totProtAT5)
mixProt1Prot4PropRSA <- fitCPA(protProfileSummary=mixProt1Prot4RSA, 
...
mixProt1Prot4PropSpecAmt <- fitCPA(protProfileSummary=mixProt1Prot4SpecAmt,
...
mixProt1Prot4PropAcup <- 
                  fitCPA(protProfileSummary=mixProt1Prot4$relAmount,
...
mixProt1Prot4PropRSALog2 <- fitCPA(protProfileSummary=log2(mixProt1Prot4RSA + eps), 
...
mixProt1Prot4PropSpecAmtLog2 <- 
            fitCPA(protProfileSummary=log2(mixProt1Prot4SpecAmt + eps),
                        markerProfiles=log2(markerProfiles + eps), numDataCols=9)
round(mixProt1Prot4PropSpecAmtLog2, digits=3)

mixProt1Prot4PropAcupLog2 <- 
            fitCPA(protProfileSummary=log2(mixProt1Prot4$relAmount + eps),
                        markerProfiles=log2(AcupMarkers + eps) , numDataCols=9)
round(mixProt1Prot4PropAcupLog2, digits=3)

```

Following are plots of the CPA estimated vs actual mixture proportions for simulated proteins where the different profiles (RSA, normalized specific amounts, and relative amounts)_ are log2-transformed.
...
mixtureAreaError(mixProtiProtjProp=mixProt1Prot4PropRSALog2, 
            NstartMaterialFractions=6,
            Loc1=i, Loc2=j, input.prop=mixProt1Prot4$input.prop)
mixtureAreaError(mixProtiProtjProp=mixProt1Prot4PropSpecAmtLog2, 
            NstartMaterialFractions=6, 
            Loc1=i, Loc2=j, input.prop=mixProt1Prot4$input.prop)
...
mixProt1Prot6Prop <- fitCPA(protProfileSummary=mixProt1Prot6RSA, 
                               markerProfiles=markerProfilesrsa, numDataCols=9)
round(mixProt1Prot6Prop, digits=3)
```
As noted earlier, the simulated proportions are estimated very accurately.

Following Vignette 3, we may see what happens if we apply the CPA routine to profiles containing  normalized specific amounts:

```{r, echo=TRUE, eval=TRUE}
mixProt1Prot6SpecAmt <- t(apply(mixProt1Prot6RSA,1, function(x) x/sum(x)))

mixProt1Prot6PropSpecAmt <- 
                    fitCPA(protProfileSummary=mixProt1Prot6SpecAmt,
                    markerProfiles=markerProfiles, numDataCols=9)
...
```{r, echo=TRUE, eval=TRUE}
mixProt1Prot6PropAcup <- fitCPA(protProfileSummary=mixProt1Prot6$relAmount,
                                   markerProfiles=AcupMarkers, numDataCols=9)
```
...
Now try a log2 transformationon different types of simulated protein and marker profiles (RSA, normalized specific amounts, and relative amounts). Note that the CPA values are different.

```{r, echo=TRUE, eval=TRUE}
eps <- 0.00005

mixProt1Prot6PropLog2 <- 
           fitCPA(protProfileSummary=log2(mixProt1Prot6RSA + eps), 
                      markerProfiles=log2(markerProfilesrsa + eps), numDataCols=9)
round(mixProt1Prot6PropLog2, digits=3)

mixProt1Prot6PropSpecAmtLog2 <- 
           fitCPA(protProfileSummary=log2(mixProt1Prot6SpecAmt + eps),
                      markerProfiles=log2(markerProfiles + eps), numDataCols=9)
round(mixProt1Prot6PropSpecAmt, digits=3)


mixProt1Prot6PropAcupLog2 <- 
           fitCPA(protProfileSummary=log2(mixProt1Prot6$relAmount + eps),
                           markerProfiles=log2(AcupMarkers + eps), numDataCols=9)
