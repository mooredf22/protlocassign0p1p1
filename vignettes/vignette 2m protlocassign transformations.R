...
$${r_{\alpha ,l}} = {{{{\mathop a\limits^\smile  }_{\alpha ,l}}} \over {{{\mathop t\limits^\smile  }_l}}} = {{{s_{\alpha ,l}} \cdot \sum\nolimits_j^k {{t_i}} } \over {\sum\nolimits_j^k {{s_{\alpha ,l}}{t_i}} }}$$
...
```{r, echo=TRUE, eval=TRUE}
mixCytoLysoRelAmtLog2 <- cbind(mixCytoLysoRelAmt[,1], log2(mixCytoLysoRelAmt[,-1] + .01))
names(mixCytoLysoRelAmtLog2)[1] <- "protName"
...
mixCytoLysoPropT <- proLocAll(protProfileSummary=mixCytoLysoRelAmtLog2,
                              markerLocR=markerLocRrsalog2,
                            n.channels=9)
mixCytoLysoPropLog <- mixCytoLysoPropT[,-1]
row.nam(mixCyestoLysoPropLog) <- mixCytoLysoPropT[,1]
