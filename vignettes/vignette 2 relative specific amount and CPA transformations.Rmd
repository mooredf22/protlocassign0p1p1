...
As explained in the Vignette 1, as an example we will use two R data sets that are included in the `protlocassign` package.  One, `tannous`, consists of row names that indicate protein identifiers, each of which is followed by data describing the identifier profile across the nine normalized specific amounts derived from a subcellular fractionation experiment. The other data set, `refLocProteinsJadot`, consists of a list of reference proteins and their associated known subcelluar compartments.  As before, to run the program, the `protlocassign` library must be installed.
...
Since there is additional information in the last two columns of `tannous` that we want to include in the new file, specifically the numbers of spectra and peptides (`Nspectra` and `Nseq`), we add them to the output as follows:
...
markerLocRuse <- cpaSetup(protProfileSummary=tannous,
