...
As explained in the Vignette 1, as an example we will use two R data sets that are included in the `protlocassign` package.  One, `protProfileNSA_AT5tmtMS2`, consists of row names that indicate protein identifiers, each of which is followed by data describing the identifier profile across the nine normalized specific amounts derived from a subcellular fractionation experiment. The other data set, `markerListJadot`, consists of a list of reference proteins and their associated known subcelluar compartments.  As before, to run the program, the `protlocassign` library must be installed.
...
refLocationProfilesNSA <- locationProfileSetup(profile=protProfileNSA_AT5tmtMS2,
...
```{r, echo=TRUE, eval=TRUE, fig.show='hold'}
refLocationProfilesRSA <- RSAfromS(SS=refLocationProfilesNSA, NstartMaterialFractions=6,
       totProt=totProtAT5)
...
for (i in 1:n.loc) {
  markerProfilePlot(refLoc=loc.list[i], markerList=markerListJadot,
                     profile=protProfileRSA_AT5tmtMS2,
...
```{r, echo=TRUE, eval=TRUE}
protCPA_AT5tmtMS2RSA <- fitCPA(profile=protProfileRSA_AT5tmtMS2,
...
protPlotfun(protName="TLN1", profile=protProfileRSA_AT5tmtMS2, numDataCols=9,
