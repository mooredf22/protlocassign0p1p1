...
The CPA method assigns each of a large number of genes to one or more of
these compartments, based on profiles from a set of reference genes.
...
```
The data set gives relative fraction levels for 7893 genes.
The last two columns give the number of spectra and the number of peptides
(sequences) for each gene. (The spectra were averaged using a
...
refLocProteinsJadot
```
There are 39 genes in this reference set.
...
We do this first with the reference profile levels in "matLocR":
