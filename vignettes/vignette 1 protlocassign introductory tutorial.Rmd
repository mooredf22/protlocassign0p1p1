...
```{r, echo=TRUE}
round(head(profilesProtAT5tmtMS2), digits=2)
```
...
markerProfilesNSA <- cpaSetup(protProfileSummary=profilesProtAT5tmtMS2, markerList=markerListJadot, numDataCols=9)
```

We display the data, using rounding to improve readability.

```{r, echo=TRUE, eval=TRUE}
round(markerProfilesNSA, digits=4)
```

To examine the available compartments, view the row names of `markerProfilesNSA`.

```{r, echo=TRUE, eval=TRUE, fig.show='hold'}
rownames(markerProfilesNSA)
```
...
refProfilePlot(refLoc="PM", markerList=markerListJadot,
                     protProfileSummary=profilesProtAT5tmtMS2,
                     markerProfiles=markerProfilesNSA)
```
...
refProfilePlot(refLoc="PM", markerList=markerListJadot,
                     protProfileSummary=profilesProtAT5tmtMS2,
                     markerProfiles=markerProfilesNSA, refProtPlot=2)
```
...
for (j in 1:5) {
  refProfilePlot(refLoc="PM", markerList=markerListJadot,
                     protProfileSummary=profilesProtAT5tmtMS2,
                     markerProfiles=markerProfilesNSA, refProtPlot=j)
  }
```
...
loc.list <- rownames(markerProfilesNSA)
n.loc <- length(loc.list)
par(mfrow=c(4,2))
for (i in 1:n.loc) {
  refProfilePlot(refLoc=loc.list[i], markerList=markerListJadot,
                     protProfileSummary=profilesProtAT5tmtMS2,
                     markerProfiles=markerProfilesNSA)
  }
```
...
protCPA_AT5tmtMS2 <- fitCPA(protProfileSummary=profilesProtAT5tmtMS2,
                            markerProfiles=markerProfilesNSA, numDataCols=9)
```
...
```{r, echo=TRUE, eval=TRUE}
round(head(protCPA_AT5tmtMS2), digits=2)
```
...
```{r, echo=TRUE, eval=TRUE}
protIndex("TLN1", protProfileSummary=profilesProtAT5tmtMS2)
```
...
protPlotfun(protName="TLN1", protProfileSummary=profilesProtAT5tmtMS2, 
numDataCols=9, n.compartments=8,
 Nspectra=T, markerProfiles=markerProfilesNSA, assignPropsMat=protCPA_AT5tmtMS2)
```                        
...
n.prots <- nrow(protCPA_AT5tmtMS2)
for (i in 1:n.prots) {
   protPlotfun(protName=rownames(protCPA_AT5tmtMS2)[i],
       protProfileSummary=profilesProtAT5tmtMS2, numDataCols=9, n.compartments=8,
       Nspectra=T, markerProfiles=markerProfilesNSA, assignPropsMat=protCPA_AT5tmtMS2)
}
dev.off()
```   
