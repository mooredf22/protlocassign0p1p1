...
We display the dat, using rounding to improve readability.

```{r, echo=TRUE, eval=TRUE}
round(markerLocRuse, digits=4)
```

To examine the available compartments, vew the row names of `markerLocRuse`.

```{r, echo=TRUE, eval=TRUE, fig.show='hold'}
rownames(markerLocRuse)
```

To view a particular compartment profile and its component proteins, use `refProfilePlot`. 
