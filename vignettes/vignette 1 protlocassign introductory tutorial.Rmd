...
markerLocRuse <- cpaSetup(protProfileSummary=tannous, refLocProteins=refLocProteinsJadot, n.channels=9)
```
...
assignPropsUse <- proLocAll(protProfileSummary=tannous,
...
```{r, echo=TRUE, eval=TRUE}
protIndex("TLN1", protProfileSummary=tannous)
```
...
protPlotfun(protName="TLN1", protProfileSummary=tannous, 
...
for (i in 1:n.prots) {
   protPlotfun(protName=rownames(assignPropsUse)[i],
       protProfileSummary=tannous, n.fractions=9, n.compartments=8,
       Nspectra=T, markerLocR=markerLocRuse, assignPropsMat=assignPropsUse)
}
dev.off()
```   
