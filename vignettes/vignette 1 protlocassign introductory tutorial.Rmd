...
```{r, echo=TRUE}
round(head(protProfileNSA_AT5tmtMS2), digits=2)
```
...
refLocationProfilesNSA_AT5tmtMS2 <- locationProfileSetup(profile=protProfileNSA_AT5tmtMS2, markerList=markerListJadot, numDataCols=9)
```

We display the data, using rounding to improve readability.

```{r, echo=TRUE, eval=TRUE}
round(refLocationProfilesNSA_AT5tmtMS2, digits=4)
```

To examine the available compartments, view the row names of `refLocationProfilesNSA_AT5tmtMS2`.

```{r, echo=TRUE, eval=TRUE, fig.show='hold'}
rownames(refLocationProfilesNSA_AT5tmtMS2)
```
...
markerProfilePlot(refLoc="PM", markerList=markerListJadot,
                     ...
                     refLocationProfiles=refLocationProfilesNSA_AT5tmtMS2)
```
...
markerProfilePlot(refLoc="PM", markerList=markerListJadot,
                     profile=protProfileNSA_AT5tmtMS2,
                     refLocationProfiles=refLocationProfilesNSA_AT5tmtMS2, refProtPlot=2)
```
...
for (j in 1:5) {
  markerProfilePlot(refLoc="PM", markerList=markerListJadot,
                     profile=protProfileNSA_AT5tmtMS2,
                     refLocationProfiles=refLocationProfilesNSA_AT5tmtMS2, refProtPlot=j)
  }
```
...
loc.list <- rownames(refLocationProfilesNSA_AT5tmtMS2)
...
protCPAfrom_NSA_AT5tmtMS2 <- fitCPA(profile=protProfileNSA_AT5tmtMS2,
                            refLocationProfiles=refLocationProfilesNSA_AT5tmtMS2, numDataCols=9)
```
...
```{r, echo=TRUE, eval=TRUE}
round(head(protCPAfrom_NSA_AT5tmtMS2), digits=2)
```
...
```{r, echo=TRUE, eval=TRUE}
protIndex("TLN1", profile=protProfileNSA_AT5tmtMS2)
```
...
```{r, echo=TRUE, eval=TRUE}
protIndex("TLN", profile=protProfileNSA_AT5tmtMS2)
```
...
protPlotfun(protName="TLN1", profile=protProfileNSA_AT5tmtMS2, 
numDataCols=9, n.compartments=8,
 Nspectra=T, refLocationProfiles=refLocationProfilesNSA_AT5tmtMS2, assignPropsMat=protCPAfrom_NSA_AT5tmtMS2)
```                        
...
n.prots <- nrow(protCPAfrom_NSA_AT5tmtMS2)
for (i in 1:n.prots) {
   protPlotfun(protName=rownames(protCPAfrom_NSA_AT5tmtMS2)[i],
       profile=protProfileNSA_AT5tmtMS2, numDataCols=9, n.compartments=8,
       Nspectra=T, refLocationProfiles=refLocationProfilesNSA_AT5tmtMS2, assignPropsMat=protCPAfrom_NSA_AT5tmtMS2)
}
dev.off()
```   
