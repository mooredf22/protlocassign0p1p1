...
Now we can run the CPA routine on the RSA-transformed levels; this may take several minutes to complete:

```{r, echo=TRUE, eval=TRUE}
assignPropsOutTemp <- proLocAll(geneProfileSummary=geneProfileSummaryRSA, matLocR=matLocRrsa,
                            n.channels=9)
ncol.a <- ncol(assignProbsOutTemp)   # last column is a convergence indicator, which should be 1
assignPropsUse <- data.frame(assignPropsOutTemp[,-ncol.a] ) # drop this last column
head(assignProbsOut)
```

We can look at the profile of, for example, the gene "AADAC" by first finding the gene number and then plotting it:
```{r, echo=TRUE, eval=TRUE}
protIndex("AADAC")
#protPlotfun(protPlot=93, assignProbsOut=assignProbsOut)
```

We shall now set up the data to examine the profiles of any gene. First, select only the genes with at least two peptides and three spectra, and extract the genes that are included:

```{r, echo=TRUE, eval=TRUE}
geneProfileSummaryUse <- subset(geneProfileSummaryTMTms2,subset={{Nspectra >= 3} & {Nseq >= 2}})  # just the data, and need sufficient spectra and peptides
genesInclude <- geneProfileSummary$geneName

```

Next, load the original data, with spectra and peptides, and select out the same genes in "genesInclude":
...
To convert to relative specific values, we convert this large dataframe in four pieces, and then reassemble:
...
finalListRSAuse3 <- rsaDirect(geneProfileLevels=finalListUse[60001:90000,3:11], 
                                   nDiffFractions=6, nNycFractions=3, totProt=totProtUse)
finalListRSAuse4 <- rsaDirect(geneProfileLevels=finalListUse[90001:nrow(finalListUse),3:11], 
                                   nDiffFractions=6, nNycFractions=3, totProt=totProtUse)
finalListRSAuseT <- rbind(finalListRSAuse1, finalListRSAuse2, finalListRSAuse3, finalListRSAuse4)

finalListRSAuse <- data.frame(finalListUse[,1:2], finalListRSAuseT)

```

#geneProfileRSAuse <- data.frame(geneProfileRSA, geneProfileSummaryUse)
protPlotfun(protPlot=93, geneProfileSummary=geneProfileSummaryRSA, finalList=finalListRSAuse, n.fractions=9,
                        Nspectra=T, matLocR=matLocRrsa)
                        
protPlotfun(protPlot=93, geneProfileSummary=assignProbsOut, finalList=finalListUse, n.fractions=9,
                        Nspectra=T, matLocR=matLocRuse, predictedProp.mat=assignPropsUse)
protPlotfun(protPlot=3, geneProfileSummary=assignProbsOut, finalList=finalListUse, n.fractions=9,
                        Nspectra=T, matLocR=matLocRuse)

Now define the names of the subcellular locations, and extract the proportionate assignments:
```{r, echo=TRUE, eval=TRUE}
...
Now make a reference set, consisting of genes assigned with a proportion >= 0.8, and add the gene names:
```{r, echo=TRUE, eval=TRUE}

catAssign80.vec <- apply(propMat, 1, assignCPAloc, cutoff=0.8, Locations=Locations)
table(catAssign80.vec)

refLocProp80all <- data.frame(geneProfileSummaryTMTms2$geneName, catAssign80.vec)
...
```{r, echo=TRUE, eval=TRUE, fig.show='hold'}
referenceProfilePlot(refLocProteins=refLocProp80, geneProfileSummary=geneProfileSummaryTMTms2,
                     matLocR=matLocR, n.channels=9)
```
...
```{r, echo=TRUE, eval=TRUE}
geneProfileSummaryMclustOnce <-  protCluster(geneProfileSummary=geneProfileSummaryTMTms2,
                                             refLocProteins=refLocProp80, refCol=1, nCol=9, clustSize=10)
...
protAbund <- abundanceTransform(matLocR,6,3, totProt=totProt)
...
protAbund <- abundanceTransform(matLocR, nDiffFractions=6, nNycFractions=3 , totProt=totProt)
...
matLocRrsa <- rsaDirect(geneProfileLevels=matLocR, nDiffFractions=6, nNycFractions=3, totProt=totProt, maxRSA=25)
