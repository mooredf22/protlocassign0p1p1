...
We see, in the plot title, that there are five reference genes. To plot, say, the second one, enter this code, using the option `refGenePlot=2`:

```{r, echo=TRUE, eval=TRUE, fig.show='hold'}

refProfilePlot(refLoc="PM", refLocProteins=refLocProteinsJadot,
                     geneProfileSummary=geneProfileSummaryTMTms2,
                     markerLocR=markerLocRuse, refGenePlot=2)
```

To show all five of the reference genes, open a window 10 inches by 7 inches. Then set up a plot array with 3 rows and 2 columns, to allow plotting all five reference genes on a page. Finally, plot them one-by-one. (The plots are not shown here.)
