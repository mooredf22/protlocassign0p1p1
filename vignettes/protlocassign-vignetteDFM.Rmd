...
Note that in R each backslash character must be doubled to be interpreted correctly (since otherwise it will be incorrecly interpreted as an escape character). Alternatively, you may use a single foward slash:
...
The first column contains the gene names, and the next 8 columns contain the allocation proportions of each gene to the eight compartments. The last two columns are the number of peptides and spectra, carried over from the input `geneProfileSummary` profile data.
...
We now consider the Christofrou data, which has 20 channels (from two combined experiments). We remove GPT_MOUSE and CPSM_MOUSE (rows 2 and 18). Because of the large number of columns, the output text wraps around to dispaly all of them.
