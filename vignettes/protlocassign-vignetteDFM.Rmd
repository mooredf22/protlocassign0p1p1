...
To use this package you will need two data sets. One consists of a column of protein names followed by a series of columns of relative protein levels derived from a subcellular fractionation experiment. (Optionally there may be two addtional columns representing the numbers of peptides and spectra that were used to compute mean protein abundance levels.) The other data set consists of a list of reference proteins in the first column and the name of the known subcellular compartment in the second column.
...
Now use the `str` function to summarize the contents of `protProfileSummaryTMTms2`:
...
Each row represents a column, and presents the variable name, type (character or number) and the first few values of that variable. The first column is the protein name and the next nine columns represent its levels of the different fractions. The last two columns, which are optional, give the number of spectra and the number of peptides (sequences) for each protein. (The spectra were averaged using a nested random effects model described in Jadot, 2016, to produce the given average for each protein.) 
...
We see, in the plot title, that there are five reference proteins. To plot, say, the second one, enter this code, using the option `refProtPlot=2`:
...
To show all five of the reference proteins, open a window 10 inches by 7 inches. Then set up a plot array with 3 rows and 2 columns, to allow plotting all five reference proteins on a page. Finally, plot them one-by-one. (The plots are not shown here.)
...
The first column contains the protein names, and the next 8 columns contain the allocation proportions of each protein to the eight compartments. The last two columns are the number of peptides and spectra, carried over from the input `protProfileSummary` profile data, which is not essential for the CPA analysis.

We can look at the profile of, for example, the protein "TLN1". We first find the protein in the dataset, and also the row number of that protein:
...
We may find the proteins with profiles nearest to a given protein using the function "nearestProteins". Distance is computed as the Euclidean distance between profiles. To use the function, we first create a distance matrix for the proteins in a list of mean profiles, such as "protProfileSummaryTMTms2". In this dataset, the profiles are in columns 2 through 10.
...
```
Finally, provide a protein name. Here, we first look for proteins in our dataset that start with "TLN". We choose the protein "AADAC", and find the 10 nearest proteins.
```{r, echo=TRUE, eval=TRUE}
protIndex("TLN")
nearestProteins("TLN1", n.nearest=10,  distProteins=distUse, protNames=protsUse)
```
