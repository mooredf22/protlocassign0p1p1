...
To see how this works in `protlocassign`, let us consider the Jadot reference protein profiles which we generate using the `locationProfileSetup` function. As in Vignette 1, we tailor the output using the `round` function.

```{r, echo=TRUE, eval=TRUE}
library(protlocassign)
refLocationProfiles <- locationProfileSetup(profile=protProfileNSA_AT5tmtMS2, 
                           markerList=markerListJadot, numDataCols=9)
round(refLocationProfiles, digits=3)
...
AcupMarkers <- relAmtTransform(NSA=refLocationProfiles, NstartMaterialFractions=6, 
                               totProt=totProtAT5)
...
```{r, echo=TRUE, eval=TRUE}
refLocationProfilesRSA_AT5tmtMS2 <- RSAfromAcup(AcupMarkers, totProt=totProtAT5)
round(refLocationProfilesRSA_AT5tmtMS2, digits=3)
```
...
```{r, echo=TRUE, eval=TRUE}
refLocationProfilesRSA_AT5tmtMS2_2 <- RSAfromNSA(NSA=refLocationProfiles,
...
```{r, echo=TRUE, eval=TRUE}
identical(refLocationProfilesRSA_AT5tmtMS2, refLocationProfilesRSA_AT5tmtMS2_2)
```
...
refLocationProfilesNSA_2 <- t(apply(
                                    refLocationProfilesRSA_AT5tmtMS2_2, 1,
                                   function(x) x/sum(x)))
identical(refLocationProfilesNSA_2, refLocationProfiles)  
```

Note that this is not identical because of internal precision issues, but is essentially equivalent.

```{r, echo=TRUE, eval=TRUE}
all.equal(refLocationProfilesNSA_2, refLocationProfiles, tolerance=0)
```
...
AcupMarkers <- relAmtTransform(NSA=refLocationProfiles, NstartMaterialFractions=6, 
                               totProt=totProtAT5)
data.frame(rownames(AcupMarkers))
...
```{r, echo=TRUE, eval=TRUE}
mixCytoLysoPropRSA <- fitCPA(profile=mixCytoLysoRSA,
                             refLocationProfiles=refLocationProfilesRSA_AT5tmtMS2,
                            numDataCols=9)
...
refLocationProfilesRSAlog2 <- log2(refLocationProfilesRSA_AT5tmtMS2 + 0.001)

mixCytoLysoPropRSALog <- fitCPA(profile=mixCytoLysoRSAlog2,
...
for (j in 2:8) {   
   ...
   mixProtiProtjProp <- fitCPA(profile=mixProtiProtjRSA,
                         refLocationProfiles=refLocationProfilesRSA_AT5tmtMS2, numDataCols=9)
...
mixErrorMat <- NULL
log2refLocationProfilesRSA <- log2(refLocationProfilesRSA_AT5tmtMS2 + 0.001)  # transform marker profiles
...
for (j in 2:8) {   # mix with each of the other compartments, and plot
   ...
   log2MixProtiProtjRSA <- log2(mixProtiProtjRSA + 0.001)
   
                     
   # Obtain CPA estimates from the RSA-transformed mixtures
   mixProtiProtjPropRSA <- fitCPA(profile=log2MixProtiProtjRSA,
                        refLocationProfiles=log2refLocationProfilesRSA, numDataCols=9)
...
Now consider using normalized specific amounts, which is what we would use for these mixtures if the amounts of starting material in each fraction were not available. The first two function calls are as above, but we obtain these normalized specific amounts by constraining the RSA values to sum to 1 before using `fitCPA` with `refLocationProfilesuse' to compute the CPA estimates.


```{r, echo=TRUE, eval=TRUE, fig.show='hold', fig.width = 7, fig.height = 10}
refLocationProfilesuse <- t(apply(refLocationProfilesRSA_AT5tmtMS2, 1, function(x) x/sum(x)))
...
for (j in 2:8) {   # mix with each of the other compartments, and plot
   ...
   mixProtiProtjProp <- fitCPA(profile=mixProtiProtjSpecific,
                        refLocationProfiles=refLocationProfilesuse, numDataCols=9)
...
Finally, we consider log2-transformed values of normalized specific amounts. This set of plots is similar to the last one except a transformation `y = log2(prop + 0.01)` is applied before obtaining CPA estimates. We calculate `refLocationProfilesuse` by constraining the row totals to be 1. 
...
mixErrorMat <- NULL
log2refLocationProfilesuse <- log2(refLocationProfilesuse + 0.001) # do this here
i=1  # fix one component of the mixture to the first, which is Cyto
for (j in 2:8) {   # mix with each of the other compartments, and plot
   #j=4
   # Create mixture of subcellular locations i and j using "Acupmarkers"
   ...
   # Now convert to normalized specific amounts
   mixProtiProtjSpecific <- t(apply(mixProtiProtjRSA,1, function(x) x/sum(x)))
   log2MixProtiProtjSpecific <- log2(mixProtiProtjSpecific + 0.001)
                     
   # Obtain CPA estimates from the RSA-transformed mixtures
   mixProtiProtjProp <- fitCPA(profile=log2MixProtiProtjSpecific,
                        refLocationProfiles=log2refLocationProfilesuse, numDataCols=9)
