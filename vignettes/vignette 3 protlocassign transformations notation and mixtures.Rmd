...
markerProfiles <- cpaSetup(protProfileSummary=profilesProtAT5tmtMS2, markerList=markerListJadot, numDataCols=9)
round(markerProfiles, digits=3)

```


Here, each row represents the profile $\tilde{s}_l$ for a protein resident solely in a particular cellular compartment. The amount of total protein derived from a set amount of starting material from all fractions in the experiment used for these vignettes is in `totProtAT5`, a vector supplied with the `protassign` package.

```{r, echo=TRUE, eval=TRUE}
round(totProtAT5, digits=3)
```
...
AcupMarkers <- relAmtTransform(SS=markerProfiles, NstartMaterialFractions=6, 
                               totProt=totProtAT5)
...
We can get the RSA matrix from `AcupMarkers` and `totProtAT5` as follows:


```{r, echo=TRUE, eval=TRUE}
markerProfilesrsa <- RSAfromAcup(AcupMarkers, totProt=totProtAT5)
round(markerProfilesrsa, digits=3)
```
...
```{r, echo=TRUE, eval=TRUE}
markerProfilesrsa_2 <- RSAfromS(SS=markerProfiles, NstartMaterialFractions=6,
        totProt=totProtAT5)

round(markerProfilesrsa_2, digits=3)
```

Finally, if we normalize an RSA profile so that the rows sum to one, this yields a normalized specific amount profile. Consider example the matrix `markerProfilesrsa`, which contains the RSA-transformed marker profiles. We normalize the rows using the `apply` function, and then transpose using the `t` function to yield a matrix of the normalized specific amounts; these values are  identical to those that we started with in `markerProfiles`:

```{r, echo=TRUE, eval=TRUE}
round(t(apply(markerProfilesrsa,1, function(x) x/sum(x))), digits=3)
```


## Simulating proteins resident in multiple subcellular locations

We may simulate data from proteins with multiple residences using the `proteinMix` function. For example, to simulate data from proteins that are distributed in a range of proportions between cytosol and lysosomes, we use this function with relative amounts (`AcupMarkers`) of their single-compartment marker profiles. We specify the mixing locations by the arguments `Loc1=1` and `Loc2=4`, which are the row numbers of the desired compartment profiles in markerProfiles:

```{r, echo=TRUE, eval=TRUE}

AcupMarkers <- relAmtTransform(SS=markerProfiles, NstartMaterialFractions=6, totProt=totProtAT5)
...
```{r, echo=TRUE, eval=TRUE}
round(mixCytoLyso$input.prop, digits=2)
```

The other component, `relAmount`,  is a matrix that contains the Acup values (relative amounts) for the simulated proteins:

```{r, echo=TRUE, eval=TRUE}
round(mixCytoLyso$relAmount, digits=3)
```

Then we can test the CPA algorithm by first converting this mixture data to RSA's:

```{r, echo=TRUE, eval=TRUE}
mixCytoLysoRSA <- RSAfromAcup(Acup=mixCytoLyso$relAmount, 
...
```{r, echo=TRUE, eval=TRUE}
mixCytoLysoPropRSA <- fitCPA(protProfileSummary=mixCytoLysoRSA, markerProfiles=markerProfilesrsa,
                            numDataCols=9)
round(mixCytoLysoPropRSA, digits=3)
```
...
For our first simulation, of proteins distributed between two compartments (Cyto and Lyso), we plot  predicted (based on CPA) and expected (based on the simulation parameters) proportional assignments using the `mixturePlot` function. This function also evaluates the prediction error by computing the area separating the predicted and expected protein mixtures via the trapezoidal rule; this is done with the `trapz` function in the `pracma` library, which must have been previously installed. We also need to tell the program which two locations were used to generate the mixtures using `Loc1` and `Loc2`. 

```{r, echo=TRUE, eval=TRUE}
library(pracma)
mixturePlot(mixProtiProtjProp=mixCytoLysoPropRSA, 
            NstartMaterialFractions=6, Loc1=1, Loc2=4,
            input.prop=mixCytoLyso$input.prop, xaxisLab=T, yaxisLab=T)
```
...
```{r, echo=TRUE, eval=TRUE}
mixCytoLysoRSAlog2 <- log2(mixCytoLysoRSA + .00005)
markerProfilesrsalog2 <- log2(markerProfilesrsa + .00005)

mixCytoLysoPropRSALog <- fitCPA(protProfileSummary=mixCytoLysoRSAlog2,
                              markerProfiles=markerProfilesrsalog2,
                            numDataCols=9)
round(mixCytoLysoPropRSALog, digits=3)
mixturePlot(mixProtiProtjProp=mixCytoLysoPropRSALog, 
...
mixErrorMat <- NULL
log2markerProfilesrsa <- log2(markerProfilesrsa + 0.01)  # transform marker profiles
...
for (j in 2:8) {   # mix with each of the other compartments, and plot
   ...
   log2MixProtiProtjRSA <- log2(mixProtiProtjRSA + 0.00005)
   
                     
   # Obtain CPA estimates from the RSA-transformed mixtures
   mixProtiProtjPropRSA <- fitCPA(protProfileSummary=log2MixProtiProtjRSA,
                        markerProfiles=log2markerProfilesrsa, numDataCols=9)
                            
   # Plot the results, including the area-based error estimate
   mixResult <- mixturePlot(mixProtiProtjProp=mixProtiProtjPropRSA, 
                            NstartMaterialFractions=6, Loc1=i, Loc2=j, 
                            input.prop=mixProtiProtj$input.prop, errorReturn = T)
   mixErrorMat <- rbind(mixErrorMat, mixResult)
...
Now consider using normalized specific amounts, which is what we would use for these mixtures if the amounts of starting material in each fraction were not available. The first two function calls are as above, but we obtain these normalized specific amounts by constraining the RSA values to sum to 1 before using `fitCPA` with `markerProfilesuse' to compute the CPA estimates.


```{r, echo=TRUE, eval=TRUE, fig.show='hold', fig.width = 7, fig.height = 10}
markerProfilesuse <- t(apply(markerProfilesrsa, 1, function(x) x/sum(x)))
...
for (j in 2:8) {   # mix with each of the other compartments, and plot
   ...
   mixProtiProtjProp <- fitCPA(protProfileSummary=mixProtiProtjSpecific,
                        markerProfiles=markerProfilesuse, numDataCols=9)
...
Finally, we consider log2-transformed values of normalized specific amounts. This set of plots is similar to the last one except a transformation `y = log2(prop + 0.01)` is applied before obtaining CPA estimates. We calculate `markerProfilesuse` by constraining the row totals to be 1. 
...
mixErrorMat <- NULL
log2markerProfilesuse <- log2(markerProfilesuse + 0.00005) # do this here
i=1  # fix one component of the mixture to the first, which is Cyto
for (j in 2:8) {   # mix with each of the other compartments, and plot
   #j=4
   # Create mixture of subcellular locations i and j using "Acupmarkers"
   ...
   # Now convert to normalized specific amounts
   mixProtiProtjSpecific <- t(apply(mixProtiProtjRSA,1, function(x) x/sum(x)))
   log2MixProtiProtjSpecific <- log2(mixProtiProtjSpecific + 0.00005)
                     
   # Obtain CPA estimates from the RSA-transformed mixtures
   mixProtiProtjProp <- fitCPA(protProfileSummary=log2MixProtiProtjSpecific,
                        markerProfiles=log2markerProfilesuse, numDataCols=9)
...
errorAll <- mixturePlotPanel(Acup=AcupMarkers, totProt=totProtAT5, errorReturn = T, 
              fitType=fitType, log2Transf=log2Transf, eps=0.00005)
dev.off()
...
For completeness we also present plots CPA mixture esimates based on relative amounts and log relative amounts. The use of relative amounts is discussed in detail in Vignette 4.
