...
markerLocR <- cpaSetup(protProfileSummary=profilesProtAT5tmtMS2, refLocProteins=refLocProteinsJadot, n.channels=9)
...
```{r, echo=TRUE, eval=TRUE}
markerLocRrsa <- RSAfromAcup(AcupMarkers, totProt=totProtAT5)
...
```{r, echo=TRUE, eval=TRUE}
mixCytoLysoPropRSA <- proLocAll(protProfileSummary=mixCytoLysoRSA, markerLocR=markerLocRrsa,
                            n.channels=9)
...
markerLocRrsalog2 <- log2(markerLocRrsa + .00005)

mixCytoLysoPropRSALog <- proLocAll(protProfileSummary=mixCytoLysoRSAlog2,
...
for (j in 2:8) {   # mix with each of the other compartments, and plot
   ...
   mixProtiProtjPropRSA <- proLocAll(protProfileSummary=log2MixProtiProtjRSA,
...
Now consider using normalized specific amounts, which is what we would use for these mixtures if the amounts of starting material in each fraction were not available. The first two function calls are as above, but we obtain these normalized specific amounts by constraining the RSA values to sum to 1 before using `proLocAll` with `markerLocRuse' to compute the CPA estimates.


```{r, echo=TRUE, eval=TRUE, fig.show='hold', fig.width = 7, fig.height = 10}
markerLocRuse <- t(apply(markerLocRrsa, 1, function(x) x/sum(x)))
...
Finally, we consider log2-transformed values of normalized specific amounts. This set of plots is similar to the last one except a transformation `y = log2(prop + 0.01)` is applied before obtaining CPA estimates. We calculate `markerLocRuse` by constraining the row totals to be 1. 
...
mixErrorMat <- NULL
log2MarkerLocRuse <- log2(markerLocRuse + 0.00005) # do this here
i=1  # fix one component of the mixture to the first, which is Cyto
for (j in 2:8) {   # mix with each of the other compartments, and plot
   #j=4
   # Create mixture of subcellular locations i and j using "Acupmarkers"
   ...
   mixProtiProtjProp <- proLocAll(protProfileSummary=log2MixProtiProtjSpecific,
                        markerLocR=log2MarkerLocRuse, n.channels=9)
